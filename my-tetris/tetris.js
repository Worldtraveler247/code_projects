...
function arenaSweep() {
    ...
    if (rowsCleared > 0) {
        // PARTY MODE: Flash the background!
        document.body.style.backgroundColor = colors[Math.floor(Math.random() * 7) + 1];
        setTimeout(() => { document.body.style.backgroundColor = '#202028'; }, 100);
    }
    updateScore();
}
...
function clearBottomRow() {
    ...
    arena.unshift(row);
    player.score += 50; // Bonus for cheating!
    updateScore();
}
...
document.addEventListener('keydown', event => {
    if (event.keyCode === 37) {
        ...
        playerRotate(1);
    } else if (event.keyCode === 67) { // 'C' key for Cheat
        clearBottomRow();
    }
});
